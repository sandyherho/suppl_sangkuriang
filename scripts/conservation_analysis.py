...
def main():
    ...
    # ========================================================================
    # Create Figure (3 panels: Mass, Momentum, Energy + summary bar)
    # Layout: 2 rows, 2 columns
    # ========================================================================
    fig = plt.figure(figsize=(7.5, 6.5))
    
    # Create grid spec for flexible layout
    gs = fig.add_gridspec(2, 2, left=0.10, right=0.98, top=0.92, bottom=0.10,
    ...
    ax1.text(0.03, 0.95, panel_labels[0], transform=ax1.transAxes,
    ...
    ax2.text(0.03, 0.95, panel_labels[1], transform=ax2.transAxes,
            fontsize=11, fontweight='bold', va='top')
    ...
    ax3.text(0.03, 0.95, panel_labels[2], transform=ax3.transAxes,
            fontsize=11, fontweight='bold', va='top')
    ...
    ax4.bar(x_pos - width, mass_errors, width, 
           label=r'$\Delta M/M_0$', color='#1f77b4', edgecolor='black', linewidth=0.5)
    ax4.bar(x_pos, momentum_errors, width,
           label=r'$\Delta P/P_0$', color='#ff7f0e', edgecolor='black', linewidth=0.5)
    ax4.bar(x_pos + width, energy_errors, width,
           label=r'$\Delta E/E_0$', color='#2ca02c', edgecolor='black', linewidth=0.5)
    ...
    ax4.legend(loc='upper left', frameon=True, edgecolor='black', fancybox=False,
               fontsize=8)
    ax4.text(0.03, 0.95, panel_labels[3], transform=ax4.transAxes,
            fontsize=11, fontweight='bold', va='top')
    ...
    with open(stats_file, 'w', encoding='utf-8') as f:
        ...
        f.write("  du/dt + epsilon * u * du/dx + mu * d³u/dx³ = 0\n\n")
        f.write("conserves three invariants:\n")
        f.write("  Mass:     M = integral(u dx)\n")
        f.write("  Momentum: P = integral(u² dx)\n")
        f.write("  Energy:   E = integral[(epsilon/2)*u³ - (3*mu/2)*(du/dx)²] dx\n\n")
        ...
        f.write("   Hamiltonian involves spatial derivatives (du/dx)².\n\n")
